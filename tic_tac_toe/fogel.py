    ...
    for parent in parents:
        ...
        if random.randint(0, 1) == 0:
            ...
            if add_subtract == "Subtract" and child_hidden_layer_size != 1:
                child_hidden_layer_size -= 1
                selected_node_num = random.choice([node.node_num for node in parent.nodes if node.node_num not in get_nodes_excluded_from_removal(parent.nodes, parent.bias_nodes)])
...
#refactor, redo removing and reformat neural_net.nodes to  dict of layers. also, change new node nums to max(all node nums) + 1